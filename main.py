...
# Функция для предсказания класса на основе текста и получения списка весов слов
def predict_class_and_get_word_weights(text, model):
    ...
    # Извлечь коэффициенты для этого класса
    coef = model.named_steps['clf'].coef_[class_index]

    # Получить список слов из CountVectorizer
    feature_names = model.named_steps['vect'].get_feature_names_out()

    # Разбить входной текст на слова
    input_words = text.split()

    # Создать словарь, связывающий слова из входного текста с их коэффициентами
...
def normalize_word_weights(word_weights):
    # Находим минимальное и максимальное значение весов слов
    min_weight = min(word_weights.values())
    max_weight = max(word_weights.values())

    # Диапазон, в который вы хотите нормализовать веса (-1 до 1)
    target_min = -1
    target_max = 1

    # Нормализуем веса слов
...
# Получить предсказанный класс и веса слов из входного текста
...
# Вывести предсказанный класс и веса слов из входного текста
